...
if model is None:
    logger.error("Could not load model from any source. Predict endpoint will be disabled.")

# Prometheus metrics
count = Counter("predict_req_tot", "Total number of prediction requests")
latency = Histogram("predict_req_latency", "Latency of prediction requests in seconds", buckets=[0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1.0, 2.0])
error_count = Counter("predict_req_errors", "Total number of prediction errors", ["status_code", "error_type"])
request_count = Counter("predict_req_by_status", "Total requests by HTTP status code", ["status_code"])
model_version_gauge = Gauge("model_version_info", "Model version information", ["model_var", "version", "loaded_from"])
...
def predict(payload: Payload):
    ...
    try:
        with latency.time():
            # Transform API input (3 features) to model input (7 features)
            X = [transform_to_model_features(row.ret_mean, row.ret_std, row.n) for row in payload.rows]
            X_array = np.array(X)
