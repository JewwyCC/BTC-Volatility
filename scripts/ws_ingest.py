...
def on_message(ws, message):
    ...
    try:
        ...
        product_id = (
            data.get("product_id") or data.get("product_ids", [None])[0]
        ...
        if config.get("data", {}).get("raw_dir"):
            ...
            raw_file = (
                raw_dir
                / f"ticks_{datetime.now(timezone.utc).strftime('%Y%m%d')}.ndjson"
            )
...
def on_open(ws):
    ...
    heartbeat_subscribe = {"type": "subscribe", "channel": "heartbeats"}
