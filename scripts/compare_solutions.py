...
def analyze_solution(config_path: str, solution_name: str):
    ...
    print(f"\nLabel distribution:")
    print(f"  Train - Positives: {y_train.sum()} ({y_train.mean()*100:.2f}%), Negatives: {(1-y_train).sum()} ({(1-y_train.mean())*100:.2f}%)")
    print(f"  Validation - Positives: {y_val.sum()} ({y_val.mean()*100:.2f}%), Negatives: {(1-y_val).sum()} ({(1-y_val.mean())*100:.2f}%)")
    print(f"  Test - Positives: {y_test.sum()} ({y_test.mean()*100:.2f}%), Negatives: {(1-y_test).sum()} ({(1-y_test.mean())*100:.2f}%)")
...
result_90 = analyze_solution('config.yaml', 'Solution 1a: 90th percentile, temporal split')
...
result_85 = analyze_solution('config.yaml', 'Solution 1b: 85th percentile, temporal split')
...
result_strat = analyze_solution('config.yaml', 'Solution 2: 95th percentile, stratified temporal split')
...
print(f"{'-'*100}")
print(f"{'Threshold percentile':<40} {result_90['threshold_percentile']:<20.0f} {result_85['threshold_percentile']:<20.0f} {result_strat['threshold_percentile']:.0f}")
print(f"{'Train positives':<40} {result_90['train_positives']:<20} {result_85['train_positives']:<20} {result_strat['train_positives']}")
print(f"{'Validation positives':<40} {result_90['val_positives']:<20} {result_85['val_positives']:<20} {result_strat['val_positives']}")
print(f"{'Test positives':<40} {result_90['test_positives']:<20} {result_85['test_positives']:<20} {result_strat['test_positives']}")
print(f"{'Can evaluate validation':<40} {result_90['can_eval_val']:<20} {result_85['can_eval_val']:<20} {result_strat['can_eval_val']}")
print(f"{'Can evaluate test':<40} {result_90['can_eval_test']:<20} {result_85['can_eval_test']:<20} {result_strat['can_eval_test']}")
