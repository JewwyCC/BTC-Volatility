...
def load_model_predictions(features_path: str, artifacts_dir: Path):
    ...
    try:
        ...
        if baseline_path.exists():
            ...
            baseline_pred = (X_test["rolling_std_returns"] >= zscore_threshold).astype(
                int
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Evidently Model Comparison Report"
    )
    parser.add_argument(
        "--features",
        type=str,
    ...
    if reference_model not in comparison_data:
        ...
        logger.warning(
            f"Reference model '{args.reference_model}' not found, using '{reference_model}'"
        )
    ...
    for model_name, current_data in comparison_data.items():
        ...
        drift_result = drift_report.run(reference_data=ref_df, current_data=curr_df)

        # Save report
        report_path = (
            output_dir / f"model_comparison_{reference_model}_vs_{model_name}.html"
        )
