...
def export_run_to_file_store(client: MlflowClient, run_id: str, experiment_id: str, 
                             file_store_path: str) -> bool:
    """
...
def migrate_experiment(client: MlflowClient, experiment_name: str, 
                      file_store_path: str) -> int:
    """
    ...
    try:
        ...
        exp_meta = {
            ...
            'tags': {k: v for k, v in experiment.tags.items()} if hasattr(experiment, 'tags') else {},
        }
        ...
        success_count = 0
        for run in runs:
            if export_run_to_file_store(client, run.info.run_id, 
                                      experiment.experiment_id, file_store_path):
                success_count += 1
...
def main():
    parser = argparse.ArgumentParser(description='Migrate MLflow runs from SQLite to file store')
    ...
    parser.add_argument('--file_store_path', type=str, default='/mlflow/mlruns',
    ...
    # Migrate experiment
    success_count = migrate_experiment(client, args.experiment_name, str(file_store_path))
