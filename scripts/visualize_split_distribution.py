...
def main():
    ...
    print(f"         = {(df['timestamp'].max() - df['timestamp'].min()).total_seconds() / 60:.1f} minutes")
    ...
    print(f"  Train - Spikes (1): {y_train.sum()} ({y_train.mean()*100:.2f}%), Normal (0): {(1-y_train).sum()} ({(1-y_train.mean())*100:.2f}%)")
    print(f"  Validation - Spikes (1): {y_val.sum()} ({y_val.mean()*100:.2f}%), Normal (0): {(1-y_val).sum()} ({(1-y_val.mean())*100:.2f}%)")
    print(f"  Test - Spikes (1): {y_test.sum()} ({y_test.mean()*100:.2f}%), Normal (0): {(1-y_test).sum()} ({(1-y_test.mean())*100:.2f}%)")
    
    # Analyze volatility distribution in each split
    print(f"\nVolatility statistics by split:")
    print(f"  Train - Mean: {df_train['future_volatility'].mean():.8f}, Std: {df_train['future_volatility'].std():.8f}, Max: {df_train['future_volatility'].max():.8f}")
    print(f"  Validation - Mean: {df_val['future_volatility'].mean():.8f}, Std: {df_val['future_volatility'].std():.8f}, Max: {df_val['future_volatility'].max():.8f}")
    print(f"  Test - Mean: {df_test['future_volatility'].mean():.8f}, Std: {df_test['future_volatility'].std():.8f}, Max: {df_test['future_volatility'].max():.8f}")
    
    # Check if validation/test have spikes at all (using their own thresholds)
    tau_val = df_val['future_volatility'].quantile(threshold_percentile) if len(df_val) > 0 else np.nan
    tau_test = df_test['future_volatility'].quantile(threshold_percentile) if len(df_test) > 0 else np.nan
    
    print(f"\nIf we used each split's own threshold:")
    if not np.isnan(tau_val):
        print(f"  Validation {threshold_percentile*100:.0f}th percentile: {tau_val:.8f}")
        print(f"    Spikes at this threshold: {(df_val['future_volatility'] >= tau_val).sum()}")
    if not np.isnan(tau_test):
        print(f"  Test {threshold_percentile*100:.0f}th percentile: {tau_test:.8f}")
        print(f"    Spikes at this threshold: {(df_test['future_volatility'] >= tau_test).sum()}")
    ...
    print(f"This threshold represents the {threshold_percentile*100:.0f}th percentile of training data's future_volatility.")
    print(f"\nValidation set statistics:")
    print(f"  Max future_volatility in validation: {df_val['future_volatility'].max():.8f}")
    print(f"  Training threshold (τ): {tau:.8f}")
    if df_val['future_volatility'].max() < tau:
        print(f"  → Validation's maximum volatility is LOWER than the training threshold!")
        ...
        print(f"  → The validation period happened to be a 'calm' period with no spikes.")
    else:
        print(f"  → Validation's maximum volatility exceeds the threshold, but no samples do.")
    ...
    ax1 = axes[0]
    ax1.scatter(df_train['timestamp'], df_train['future_volatility'], 
                alpha=0.3, s=10, label='Train', color='blue')
    ax1.scatter(df_val['timestamp'], df_val['future_volatility'], 
                alpha=0.3, s=10, label='Validation', color='orange')
    ax1.scatter(df_test['timestamp'], df_test['future_volatility'], 
                alpha=0.3, s=10, label='Test', color='green')
    ax1.axhline(tau, color='red', linestyle='--', linewidth=2, label=f'Threshold (τ={tau:.8f})')
    ...
    ax1.axvline(df_train['timestamp'].max(), color='black', linestyle=':', linewidth=1, alpha=0.5)
    ax1.axvline(df_val['timestamp'].max(), color='black', linestyle=':', linewidth=1, alpha=0.5)
    ax1.text(df_train['timestamp'].max(), ax1.get_ylim()[1]*0.95, 'Train/Val', 
             rotation=90, va='top', ha='right', fontsize=8)
    ax1.text(df_val['timestamp'].max(), ax1.get_ylim()[1]*0.95, 'Val/Test', 
             rotation=90, va='top', ha='right', fontsize=8)
    
    # Plot 2: Histogram of future_volatility by split
    ax2 = axes[1]
    ax2.hist(df_train['future_volatility'], bins=50, alpha=0.5, label='Train', color='blue', density=True)
    ax2.hist(df_val['future_volatility'], bins=50, alpha=0.5, label='Validation', color='orange', density=True)
    ax2.hist(df_test['future_volatility'], bins=50, alpha=0.5, label='Test', color='green', density=True)
    ax2.axvline(tau, color='red', linestyle='--', linewidth=2, label=f'Threshold (τ={tau:.8f})')
    ...
    ax3.scatter(df_train['timestamp'], [1]*len(df_train), alpha=0.1, s=5, color='blue', label='Train samples')
    ax3.scatter(df_val['timestamp'], [1]*len(df_val), alpha=0.1, s=5, color='orange', label='Validation samples')
    ax3.scatter(df_test['timestamp'], [1]*len(df_test), alpha=0.1, s=5, color='green', label='Test samples')
    
    if len(train_spikes) > 0:
        ax3.scatter(train_spikes['timestamp'], [1.1]*len(train_spikes), 
                   s=50, marker='^', color='blue', label=f'Train spikes ({len(train_spikes)})', alpha=0.7)
    if len(val_spikes) > 0:
        ax3.scatter(val_spikes['timestamp'], [1.1]*len(val_spikes), 
                   s=50, marker='^', color='orange', label=f'Validation spikes ({len(val_spikes)})', alpha=0.7)
    if len(test_spikes) > 0:
        ax3.scatter(test_spikes['timestamp'], [1.1]*len(test_spikes), 
                   s=50, marker='^', color='green', label=f'Test spikes ({len(test_spikes)})', alpha=0.7)
    
    ax3.axvline(df_train['timestamp'].max(), color='black', linestyle=':', linewidth=1, alpha=0.5)
    ax3.axvline(df_val['timestamp'].max(), color='black', linestyle=':', linewidth=1, alpha=0.5)
