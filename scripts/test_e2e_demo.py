...
def step_6_test_api_endpoints() -> Dict[str, bool]:
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print_success(f"/version: model={data.get('model')}, version={data.get('version')}")
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print_success(f"/predict: {len(data.get('scores', []))} predictions returned")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_warning(f"/metrics returned status {response.status_code} (may not be implemented)")
