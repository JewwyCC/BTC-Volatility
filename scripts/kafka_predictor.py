...
def load_model_from_mlflow(config: Dict, project_root: Path) -> Optional[object]:
    ...
    try:
        ...
        if experiment is None:
            logger.warning(
                f"Experiment '{experiment_name}' not found in MLflow, falling back to local artifacts"
            )
            return load_model_from_local(project_root, model_type)
        ...
        if model_source == "run_id" and run_id:
            ...
            if runs.empty:
                logger.warning(
                    f"No runs found in experiment '{experiment_name}', falling back to local artifacts"
                )
                return load_model_from_local(project_root, model_type)
            ...
            logger.warning(
                f"Invalid model_source: {model_source}, falling back to local artifacts"
            )
            return load_model_from_local(project_root, model_type)
        ...
        try:
            ...
            logger.warning(
                f"Could not load model from MLflow: {e}, falling back to local artifacts"
            )
            return load_model_from_local(project_root, model_type)

    except Exception as e:
        logger.warning(
            f"Error connecting to MLflow: {e}, falling back to local artifacts"
        )
        return load_model_from_local(project_root, model_type)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Cannot convert score to float: {raw_score} (type: {type(raw_score)}), error: {e}"
                )
                return None
            ...
            try:
                ...
                logger.error(
                    f"Score is not a valid number: {score} (type: {type(score)})"
                )
                return None

        except (IndexError, TypeError, ValueError, AttributeError) as e:
            logger.error(
                f"Error extracting score from probability array: {e}, proba shape: {proba.shape if hasattr(proba, 'shape') else 'N/A'}"
            )
            return None
        ...
        try:
            ...
            logger.error(
                f"Cannot compare score {score} (type: {type(score)}) with 0.5: {e}"
            )
            return None

        return {"prediction": int(prediction), "score": score, "probability": score}
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--topic_in",
        type=str,
        default="ticks.features",
        help="Input Kafka topic (features)",
    )
    parser.add_argument(
        "--topic_out",
        type=str,
        default="ticks.predictions",
    ...
    if model is None:
        logger.error(
            "Failed to load model from both MLflow and local artifacts. Exiting."
        )
        return 1
    ...
    try:
        ...
        for message in consumer:
            ...
            try:
                ...
                if features is None:
                    logger.warning(
                        f"Could not transform features from message {message_count}"
                    )
                    error_count += 1
                    continue
                ...
                if prediction_result is None:
                    logger.warning(
                        f"Could not make prediction for message {message_count}"
                    )
                    error_count += 1
                    continue
                ...
                try:
                    ...
                    if prediction_count % 100 == 0:
                        logger.info(
                            f"Processed {prediction_count} predictions (total messages: {message_count}, errors: {error_count})"
                        )
                except Exception as e:
        ...
        if prediction_count > 0:
            logger.info(
                f"Average throughput: {prediction_count/elapsed_time:.2f} predictions/sec"
            )
        logger.info("=" * 60)
