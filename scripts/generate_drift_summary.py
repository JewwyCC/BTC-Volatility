...
def generate_drift_summary(df_reference, df_current, feature_cols, output_dir):
    ...
    try:
        ...
        try:
            ...
            report_dict = {"metrics": []}
            logger.warning("Could not extract full report dict, using minimal structure")
        ...
        for metric in metrics_list:
            if isinstance(metric, dict):
                ...
                if "DataDrift" in metric_name or metric_name == "DataDriftTable":
                    ...
                    summary["number_of_drifted_features"] = result.get("number_of_drifted_features", 0)
                    summary["share_of_drifted_features"] = result.get("share_of_drifted_features", 0.0)
                    ...
                    for feature_name, feature_data in features.items():
                        if isinstance(feature_data, dict):
                            drifted = feature_data.get(" drift_detected", False) or feature_data.get("drift_detected", False)
                            if drifted:
                                summary["drifted_features"].append({
                                    "name": feature_name,
                                    "drift_score": feature_data.get("drift_score", 0.0),
                                    "statistical_test": feature_data.get("statistical_test", "unknown")
                                })
                                
                                # Store feature statistics
                                summary["feature_stats"][feature_name] = {
                                    "drift_score": feature_data.get("drift_score", 0.0),
                                    "reference_mean": feature_data.get("reference_mean"),
...
"""
        for feature in summary["drifted_features"]:
            ...
            change = "N/A"
            if isinstance(ref_mean, (int, float)) and isinstance(curr_mean, (int, float)):
                if ref_mean != 0:
...
def main():
    ...
    parser.add_argument('--summary_path', type=str, default='docs/drift_summary.md',
    ...
    logger.info(f"  Current (Test): {len(df_current)} samples ({(1-args.train_pct-args.val_pct)*100:.1f}%)")
    ...
    logger.info(f"Share of Drifted Features: {summary['share_of_drifted_features']:.1%}")
    
    if summary['drifted_features']:
        logger.info("\nDrifted Features:")
        for feature in summary['drifted_features']:
            logger.info(f"  - {feature['name']}: drift_score={feature['drift_score']:.4f}")
