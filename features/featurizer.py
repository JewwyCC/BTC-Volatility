...
def process_message_buffer(buffer: List[Dict], config: Dict, producer: Optional[KafkaProducer] = None) -> pd.DataFrame:
    ...
    try:
            ...
            if ticker_data:
                ...
                if len(buffer) >= args.buffer_size:
                    ...
                    if len(df_features) > 0:
                        ...
                        logger.info(f"Processed {len(buffer)} messages, generated {len(df_features)} features (total: {feature_count})")
                    
                    # Clear buffer (keep last few for continuity)
                    buffer = buffer[-100:]  # Keep last 100 for rolling window continuity
            
            # Log progress
            if message_count % 1000 == 0:
                logger.info(f"Processed {message_count} messages, generated {feature_count} features")
        ...
        if len(buffer) > 0:
            ...
            if len(df_features) > 0:
                all_features.append(df_features)
                feature_count += len(df_features)
                logger.info(f"Final processing: {len(buffer)} messages, {len(df_features)} features")
        ...
        if all_features:
            ...
            logger.info(f"  Time range: {df_all['timestamp'].min()} to {df_all['timestamp'].max()}")
