...
def baseline_zscore_model(X: pd.DataFrame, y: pd.Series, threshold_std: float = 2.0, 
                          precomputed_params: dict = None) -> dict:
    """
    ...
    y_pred_proba = 1 / (1 + np.exp(-(z_scores - threshold_std)))  # Sigmoid transformation
...
def evaluate_model(y_true: pd.Series, y_pred: pd.Series, y_pred_proba: pd.Series, 
                  model_name: str, split_name: str) -> dict:
    ...
    if 1 not in unique_classes:
        logger.warning(f"{model_name} - {split_name.upper()}: No positive labels (spikes) found. Metrics may be limited.")
...
def stratified_temporal_split(df: pd.DataFrame, tau: float, train_pct: float = 0.7,
    ...
    if len(val_positives) < target_val_pos:
        ...
        if len(train_positives) > target_train_pos:
            ...
            logger.info(f"  Swapped {len(to_move)} positives from train to val (maintaining temporal order)")
        # If training doesn't have enough, try test set
            ...
            logger.info(f"  Swapped {len(to_move)} positives from test to val (maintaining temporal order)")
    
    # For test set
    if len(test_positives) < target_test_pos and len(train_positives) > target_train_pos:
        ...
        logger.info(f"  Swapped {len(to_move)} positives from train to test (maintaining temporal order)")
    ...
    if len(test_positives) < target_test_pos and len(val_positives) > target_val_pos:
        ...
        logger.info(f"  Swapped {len(to_move)} positives from val to test (maintaining temporal order)")
    
    # Recombine splits
    df_train = pd.concat([train_positives, train_negatives]).sort_values('timestamp').reset_index(drop=True)
    df_val = pd.concat([val_positives, val_negatives]).sort_values('timestamp').reset_index(drop=True)
    df_test = pd.concat([test_positives, test_negatives]).sort_values('timestamp').reset_index(drop=True)
...
def main():
    ...
    parser.add_argument('--mlflow_uri', type=str, default='http://localhost:5001',
                       help='MLflow tracking URI')
    parser.add_argument('--experiment_name', type=str, default='volatility_detection',
    ...
    with mlflow_context:
        ...
        val_result = baseline_zscore_model(X_val, y_val, threshold_std=2.0, 
                                           precomputed_params=baseline_params)
        ...
        test_result = baseline_zscore_model(X_test, y_test, threshold_std=2.0,
                                           precomputed_params=baseline_params)
        y_test_pred_baseline = test_result['predictions']
        y_test_proba_baseline = test_result['probabilities']
        
        # Metrics
        metrics_val = evaluate_model(y_val, y_val_pred_baseline, y_val_proba_baseline, 
                                     "Baseline", "val")
        metrics_test = evaluate_model(y_test, y_test_pred_baseline, y_test_proba_baseline,
                                     "Baseline", "test")
    ...
    with mlflow_context:
        ...
        if y_val.sum() > 0:
            optimal_threshold = find_optimal_threshold(y_val, y_val_pred_proba, metric='f1')
            logger.info(f"Optimal threshold (from validation set): {optimal_threshold:.4f}")
        elif y_train.sum() > 0:
            # Use training set, but be aware of potential distribution shift
            # For imbalanced data, we want to balance precision and recall
            optimal_threshold = find_optimal_threshold(y_train, y_train_pred_proba, metric='f1')
            logger.info(f"Optimal threshold (from training set, val has no positives): {optimal_threshold:.4f}")
            logger.warning("WARNING: Using training set for threshold selection. Test set may have different distribution.")
            
            # Also compute what the test-optimal threshold would be (for diagnostic purposes only)
            if y_test.sum() > 0:
                test_optimal_threshold = find_optimal_threshold(y_test, y_test_pred_proba, metric='f1')
                logger.info(f"  (Diagnostic: Test-optimal threshold would be {test_optimal_threshold:.4f})")
                logger.info(f"  (Diagnostic: This suggests distribution shift - test has {y_test.mean()*100:.1f}% positives vs train {y_train.mean()*100:.1f}%)")
        else:
            optimal_threshold = 0.5
            logger.warning("No positive labels in train/val, using default threshold 0.5")
        ...
        # Evaluate
        metrics_train = evaluate_model(y_train, y_train_pred, y_train_pred_proba,
                                      f"ML-{args.model_type}", "train")
        metrics_val = evaluate_model(y_val, y_val_pred, y_val_pred_proba,
                                    f"ML-{args.model_type}", "val")
        metrics_test = evaluate_model(y_test, y_test_pred, y_test_pred_proba,
                                     f"ML-{args.model_type}", "test")
        
        # Also evaluate test at 0.5 threshold for comparison
        if y_test.sum() > 0:
            metrics_test_05 = evaluate_model(y_test, y_test_pred_05, y_test_pred_proba,
                                           f"ML-{args.model_type}", "test_05_threshold")
