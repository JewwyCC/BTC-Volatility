...
def load_baseline_model(artifacts_dir: Path) -> Dict:
    ...
    logger.info(
        f"Loaded baseline model parameters: threshold={params['threshold']:.6f}"
    )
    return params
    ...
    if model_type == "xgboost" and XGBOOST_AVAILABLE:
        ...
        if not model_path.exists():
            raise FileNotFoundError(
                f"Logistic Regression model not found at {model_path}"
            )
        with open(model_path, "rb") as f:
            model = pickle.load(f)
...
def baseline_predict(X: pd.DataFrame, params: Dict) -> Tuple[np.ndarray, np.ndarray]:
    ...
    y_pred_proba = np.clip(
        (X["rolling_std_returns"] - mean_std) / (std_std + 1e-8), 0, 1
    )
...
def main():
    ...
    parser.add_argument(
        "--test_latency", action="store_true", help="Test inference latency"
    )
    ...
    if args.test_latency:
        ...
        if inference_time < max_allowed_time:
            logger.info(
                f"✓ Latency requirement met: {inference_time:.2f}s < {max_allowed_time:.2f}s"
            )
        else:
            logger.warning(
                f"⚠ Latency requirement not met: {inference_time:.2f}s >= {max_allowed_time:.2f}s"
            )
    ...
    logger.info(
        f"Predictions - Normal: {(1-y_pred).sum()} ({(1-y_pred.mean())*100:.2f}%)"
    )
