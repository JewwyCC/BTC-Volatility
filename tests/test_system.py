...
def check_services_running():
    ...
    try:
        ...
        required_containers = {
            'kafka': ['kafka'],
            'mlflow': ['mlflow'],
            'api': ['ml-prediction-api', 'api']  # Container name is 'ml-prediction-api'
        }
        ...
        for service_key, possible_names in required_containers.items():
            ...
            if found:
                actual_name = next((n for n in possible_names if n in running_containers), None)
                print_status(f"Service '{service_key}' is running (container: {actual_name})", "PASS")
            else:
                print_status(f"Service '{service_key}' is not running (checked containers: {possible_names})", "WARN")
                # Don't fail if API isn't running - it might be run separately
                if service_key == 'api':
                    print_status("  Note: API can be run separately with: python -m uvicorn api_v1:api --reload", "INFO")
                else:
                    all_running = False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "model" in data and "sha" in data:
                print_status(f"/version returns: model={data.get('model')}, sha={data.get('sha')}", "PASS")
                return True
            else:
...
def test_predict_endpoint(base_url: str = "http://localhost:8000") -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_status(f"/predict returns correct format: scores={data['scores']}, model_variant={data['model_variant']}, ts={data['ts']}", "PASS")
            return True
        elif response.status_code == 503:
            print_status("/predict returned 503 - Model not loaded", "WARN")
            return False
        else:
            print_status(f"/predict returned status {response.status_code}: {response.text}", "FAIL")
            return False
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(found_metrics) >= 1:
                print_status(f"/metrics returns Prometheus format (found: {len(found_metrics)} metrics)", "PASS")
                return True
            else:
...
def test_error_handling(base_url: str = "http://localhost:8000") -> bool:
    ...
    try:
        ...
        if response.status_code in [400, 422]:
            ...
            print_status(f"Unexpected status for invalid payload: {response.status_code}", "WARN")
            return True
    except Exception as e:
        print_status(f"Error testing error handling: {e}", "WARN")
        return True

def test_multiple_predictions(base_url: str = "http://localhost:8000", count: int = 5) -> bool:
    ...
    for i in range(count):
        try:
            ...
            if response.status_code == 200:
                ...
                print_status(f"Prediction {i+1} failed with status {response.status_code}", "WARN")
        except Exception as e:
    ...
    if len(results) == count:
        ...
        if len(model_variants) == 1:
            print_status(f"All {count} predictions consistent (model_variant: {list(model_variants)[0]})", "PASS")
            return True
        else:
...
def wait_for_service(base_url: str = "http://localhost:8000", max_wait: int = 30) -> bool:
