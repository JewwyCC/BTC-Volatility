    ...
    try:
        ...
        for idx, run in runs.iterrows():
            ...
            try:
                ...
                print(f"✓ Model successfully loaded from MLflow (XGBoost, run {run_id[:8]})")
                model_loaded = True
                break
                ...
                try:
                    ...
                    print(f"✓ Model successfully loaded from MLflow (sklearn, run {run_id[:8]})")
                    model_loaded = True
                    break
...
def test_api_model_loading():
    ...
    try:
        ...
        try:
            ...
            print(f"⚠ Prediction test failed (expected if API uses simplified format): {e}")
            # This is OK - the API might use a different format
            print("  Note: API predict endpoint uses simplified format (ret_mean, ret_std, n)")
...
def test_kafka_connectivity():
    ...
    try:
        ...
        try:
            ...
            print("  Note: Kafka may not be running. Start with: docker compose -f docker/compose.yaml up -d")
...
def test_config_file():
    ...
    try:
        ...
        # Check MLflow config
        mlflow_config = config.get('mlflow', {})
        required_mlflow = ['tracking_uri', 'experiment_name', 'model_source', 'model_type']
